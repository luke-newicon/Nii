...
<li data-id="<?php echo $template->id; ?>" class="template <?php echo $selected; ?> line pts pbs" onmouseover="$(this).addClass('hover')" onmouseout="$(this).removeClass('hover')">
	<div class="display line">
		...
		<div class="lastUnit txtR templateFuns">
			<a style="display:inline-block;" data-tip="{gravity:'sw'}" title="Edit" onclick="$(this).closest('.display').hide().closest('.template').find('.editForm').show().find('input').focus();return false;" href="#"><span class="icon fugue-pencil mrn"></span></a>
