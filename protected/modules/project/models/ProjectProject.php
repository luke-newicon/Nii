...
class ProjectProject extends NActiveRecord {
    ...
    /**
     * Returns the information on the amount of time spent on each time record
     * type for a specified time record type.
     * @param int $projectId The id of the project which the data should relate to.
     */
    public function projectTimeOverviewTimeType($projectId) {
	$condition = array(
	    ...
	    'condition' => 't.id =' . $projectId
	);
	return $this->model()->findAll($condition);
    }
    
       public function projectTimeOverviewTaskType($projectId) {
	$condition = array(
