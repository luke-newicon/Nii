...
class RecurlyAccount
{
	...
	{
		Yii::beginProfile('send request');
		$uri = RecurlyClient::PATH_ACCOUNTS . urlencode($accountCode);
		$result = RecurlyClient::__sendRequest($uri, 'GET');
		Yii::endProfile('send request');
		
		if (preg_match("/^2..$/", $result->code)) {
			Yii::beginProfile('parse xml');
			$ret =  RecurlyClient::__parse_xml($result->response, 'account');
			Yii::endProfile('parse xml');
			return $ret;
